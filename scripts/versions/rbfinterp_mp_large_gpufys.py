...
def interpolate_volume_elements_to_linear_time_alglib(full_volumes_data_arr, \
    ...
    data_flattened = flatten_tensor_alglib(data)
    ...
    #grid = np.hstack(tuple(grids))
    
    timegrids = [np.arange(time_idx,time_idx+1,1/interval_duration) \
    for time_idx, interval_duration in enumerate(intervals_between_volumes_t)]
    
    TI = np.hstack(tuple(timegrids))
    
    tdiminterp = len(TI)
    
    data_interpolated = np.empty((tdiminterp,zdim,ydim,xdim), dtype=np.float32)
    
    spatialgrid = flatten_tensor(data_interpolated)
    ...
    model = xalglib.rbfcreate(4, 1)
    
    xalglib.rbfsetpoints(model, data_flattened)
    ...
    #buf = xalglib.rbfcreatecalcbuffer(model)
    #"""
    for ti, t in enumerate(TI):
        for z in ZI:
            for y in YI:
                for x in XI:
                    #print(y)
                    f = xalglib.rbfcalc(model, [t, z, y, x])
                    data_interpolated[ti, np.int(z), np.int(y), np.int(x)] = f[0]
                    #print(f)
    #"""
    #f = xalglib.rbfcalc(model, [TI.tolist(), ZI.tolist(), YI.tolist(), XI.tolist()])
    #print(f)
