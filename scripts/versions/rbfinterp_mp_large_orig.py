...
def interpolate_volume_elements_to_linear_time_gpu(calculate_default_epsilon_rbf_multiquadric_4D, \
                                               kernel_func, \
    ...
    epsilon = calculate_default_epsilon_rbf_multiquadric_4D(x)
    ...
    K_xx = kernel_func(x, x, epsilon=epsilon)
    ...
    data_interpolated = data_interpolated.view(np.sum(intervals_between_volumes_t), zdim, ydim, xdim).cpu().numpy()
...
def interpolate_subvol(z_interval_t, \
    ...
    if interpolate_subvol.usegpu:
        data_interpolated = \
        interpolate_volume_elements_to_linear_time_gpu(interpolate_subvol.calculate_default_epsilon_rbf_multiquadric_4D, \
                                                   interpolate_subvol.multiquadric_kernel, \
...
def interpolate_subvol_init(volumes_data_arr, \
    ...
    interpolate_subvol.calculate_default_epsilon_rbf_multiquadric_4D = calculate_default_epsilon_rbf_multiquadric_4D
    
    interpolate_subvol.usegpu = usegpu
...
if __name__ == "__main__":
    ...
    CLI.add_argument(
      "--usegpu",
      help="1: use GPU (pykeops), 0: use CPU (scipy)",
      type=int,
      default=0,
    )
